...
class db():
    def get_class(id, connection):
        better_print("🐰 RabbitMQ > Connecting to user_request_queue")

        channel = connection.channel()

        better_print("🐰 RabbitMQ > Sending request to user_request_queue")
        ...
        def callback(ch, method, properties, body):
            ...
            better_print("🐰 RabbitMQ > Received data from user_queue")
            global data
            data = json.loads(body)

        channel.basic_consume(queue='user_queue', on_message_callback=callback, auto_ack=True)
        better_print("🐰 RabbitMQ > Consuming from user_queue")
    ...
    def save_absence_scan(records, connection, scan_id):
        if len(records) > 0:
            better_print("🐰 RabbitMQ > Connecting to absence_queue")
            channel = connection.channel()
            records_dict = {
                "scan_id": scan_id,
                "absences": records
            ...
            better_print("🐰 RabbitMQ > Sending data to absence_queue")
